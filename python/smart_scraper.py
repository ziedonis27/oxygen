#!/usr/bin/env python3
"""
Gudrs Dataset Scraper — lejuplādē JSON/JSONL/Parquet datus no HuggingFace,
GitHub un citiem avotiem ar anti-bloķēšanas paņēmieniem.

Lietošana:
    python smart_scraper.py --url "https://..." --output ./dati
    python smart_scraper.py --url "hf://username/dataset" --output ./dati
    python smart_scraper.py --url "https://huggingface.co/datasets/..." --output ./dati --hf-token hf_xxx
...
# --- Anti-bloķēšanas User-Agent rotācija ---
USER_AGENTS = [
...
def download_huggingface(url: str, output_dir: str, hf_token: str = "", split: str = "train", max_rows: int = 0):
    ...
    try:
        ...
        print(f"Kludas ar pilnu dataset, mēģinu streaming: {e}")
        try:
...
# ────────────────────────────────────────────
# Tiešā faila lejuplāde ar retry + UA rotāciju
# ────────────────────────────────────────────
def download_direct(url: str, output_dir: str, hf_token: str = "", retries: int = 3):
    ...
    print(f"📥 Tiešā lejuplāde: {filename}")
    print(f"   URL: {url[:80]}...")
                ...
                with open(out_path, "wb") as f:
                    while True:
                        ...
                        if total > 0:
                            ...
                            print(f"\r  Lejuplādē: {mb:.1f} MB ({pct:.0f}%)", end="", flush=True)

            print()
            size_mb = os.path.getsize(out_path) / 1024 / 1024
            print(f"✅ Saglabāts: {out_path} ({size_mb:.2f} MB)")
            return out_path

        except urllib.error.HTTPError as e:
            if e.code == 401:
                print(f"  Kludas 401: Vajag --hf-token autentifikācijai!")
                sys.exit(1)
            elif e.code == 403:
                print(f"  Kludas 403: Piekļuve liegta. Mēģina ar citu UA...")
            elif e.code == 429:
                wait = 5 * (attempt + 1)
                print(f"  Rate limit! Gaidu {wait}s...")
                time.sleep(wait)
            else:
                print(f"  HTTP kludas {e.code}: {e.reason}")

        except Exception as e:
            print(f"  Kludas (mēģinājums {attempt+1}): {e}")

        if attempt < retries - 1:
            random_delay(2, 5)

    print("❌ Neizdevās lejuplādēt pēc visiem mēģinājumiem.")
    sys.exit(1)
...
def download_github(url: str, output_dir: str):
    ...
    if "github.com" in url and "/blob/" in url:
        ...
        # Tas ir mape — lejuplādē visu repo kā ZIP
        print("ℹ️  GitHub mape — lejuplādē kā ZIP...")
...
def download_hf_api(url: str, output_dir: str, hf_token: str = "", max_rows: int = 1000):
    ...
    size_mb = os.path.getsize(out_path) / 1024 / 1024
    print(f"✅ Saglabāts: {out_path} ({len(records)} ieraksti, {size_mb:.2f} MB)")

# ────────────────────────────────────────────
# Galvenā funkcija
# ────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Gudrs Dataset Scraper")
    parser.add_argument("--url",       required=True,  help="URL vai HuggingFace dataset ID")
    parser.add_argument("--output",    default=".",    help="Izvades mape")
    ...
    parser.add_argument("--max-rows",  type=int, default=0, help="Maks. ierakstu skaits (0=visi)")
    parser.add_argument("--retries",   type=int, default=3, help="Mēģinājumu skaits")
    ...
    print(f"  Izvade : {args.output}")
    if args.max_rows > 0:
        print(f"  Maks.  : {args.max_rows} ieraksti")
    print(f"{'='*50}\n")
    ...
    print(f"\n{'='*50}")
    print(f"  Pabeigts! Faili saglabāti: {args.output}")
    print(f"{'='*50}\n")
