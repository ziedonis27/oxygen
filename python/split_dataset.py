#!/usr/bin/env python3
"""
Train split — splits a dataset into train/validation/test parts.
"""
import argparse
import json
import os
...
def main():
    ...
    if abs(total_pct - 100.0) > 0.1:
        print(f"Error: percentages ({total_pct:.1f}%) do not add up to 100%")
        sys.exit(1)
    ...
    for name, data in splits:
        if not data:
            print(f"  {name:12s}: 0 records (skipped)")
            continue
        ...
        print(f"  {name:12s}: {len(data):6d} records ({pct_real:.1f}%) → {os.path.basename(out_path)} ({size_mb:.2f} MB)")
